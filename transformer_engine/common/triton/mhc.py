...
def _mhc_aggregate_bwd(
    ...
    x_ptr,  # (M, C, n)
    grad_x_ptr,  # # (M, C, n)
    ws_grad_H_pre_ptr,  # Temporary workspace for grad_H_pre with shape (M, BLOCK_NUM_C * n), or None if DETERMINISTIC is False
    M,
    C,
    n: tl.constexpr,
    ...
    tl.assume(stride_ws_grad_H_pre_m % 4 == 0)
    tl.assume(stride_ws_grad_H_pre_c == 1)
    ...
    if DETERMINISTIC:
        offs_ws_H_pre_m = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
        offs_ws_H_pre_c = pid_c * n + tl.arange(0, n)
        ws_grad_H_pre_ptrs = (
            ws_grad_H_pre_ptr
            + offs_ws_H_pre_m * stride_ws_grad_H_pre_m
            + offs_ws_H_pre_c * stride_ws_grad_H_pre_c
        )
