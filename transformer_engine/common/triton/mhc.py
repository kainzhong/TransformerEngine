...
def elementwise_config():
    ...
    for m, w, s in itertools.product(block_m, warps, stages):
        configs.append(
            triton.Config({'BLOCK_SIZE_M': m}, num_warps=w, num_stages=s)
        )
...
def _mhc_elementwise_fwd_fused(
    ...
    a = tl.load(a_ptr + offs_a * stride_a, mask=offs_a < N, other=0.0) # a[2*n + n*n:] is filled with garbage
    ...
    h = tl.load(h_ptr + offs_m[:, None] * stride_hm + cols[None, :] * stride_hn, mask=mask_m[:, None], other=0.0)  # (BLOCK_SIZE_M, BLOCK_SIZE_N)

    h = a[None, :] * h 
    h = tl.fma(h, 1.0 / r[:, None], b[None, :]) # (BLOCK_SIZE_M, BLOCK_SIZE_N), where the first 2n columns are H_pre and H_post, and the rest are H_res
    ...
    tl.store(out_ptr + offs_m[:, None] * stride_out_m + cols[None, :] * stride_out_n, h, mask=mask_m[:, None])
...
def _mhc_elementwise_bwd_fused(
    grad_out_ptr, out_ptr, # (M, 2n + n^2), which is padded to (M, 32) in the last dimension
    ...
    stride_out_m, stride_out_n,
    stride_grad_hm, stride_grad_hn,
    stride_hm, stride_hn,
    stride_grad_a, stride_a,
    stride_grad_b,
    stride_grad_r, stride_r,
    BLOCK_SIZE_M: tl.constexpr,
    BLOCK_SIZE_N: tl.constexpr,
):
    pid = tl.program_id(0)
    ...
    offs_a = tl.where((cols >= 2*n) & (cols < 2*n + n*n), 2, offs_a)
    # Pick a[0] from a for the first 4 columns, a[1] for the next 4 columns, and a[2] for the rest of the columns
    a = tl.load(a_ptr + offs_a * stride_a, mask=offs_a < 3, other=0.0) # a[2*n + n*n:] is filled with garbage
    a = tl.where(cols < N, a, 0.0) # Mask out the garbage values in a
    ...
    grad_out = tl.load(grad_out_ptr + offs_m[:, None] * stride_grad_out_m + cols[None, :] * stride_grad_out_n, mask=mask_m[:, None] & mask_n[None, :], other=0.0) # (BLOCK_SIZE_M, BLOCK_SIZE_N)
    out = tl.load(out_ptr + offs_m[:, None] * stride_out_m + cols[None, :] * stride_out_n, mask=mask_m[:, None] & mask_n[None, :], other=0.0) # (BLOCK_SIZE_M, BLOCK_SIZE_N)
    h = tl.load(h_ptr + offs_m[:, None] * stride_hm + cols[None, :] * stride_hn, mask=mask_m[:, None] & mask_n[None, :], other=0.0) # (BLOCK_SIZE_M, BLOCK_SIZE_N)
    ...
    tl.atomic_add(grad_a_ptr+stride_grad_a, tl.where((cols[None, :] >= n) & (cols[None, :] < 2*n), grad_a, 0.0).sum())
    tl.atomic_add(grad_a_ptr+2*stride_grad_a, tl.where((cols[None, :] >= 2*n) & (cols[None, :] < 2*n + n*n), grad_a, 0.0).sum())
    ...
    tl.store(grad_h_ptr + offs_m[:, None] * stride_grad_hm + cols[None, :] * stride_grad_hn, grad_h, mask=mask_m[:, None] & mask_n[None, :])
...
def sinkhorn_config():
    ...
    for b, w, s in itertools.product(block, warps, stages):
        configs.append(
            triton.Config({'BLOCK_SIZE': b}, num_warps=w, num_stages=s)
        )
    if os.environ.get("TRITON_SKIP_AUTOTUNING", "0") == "1":
        configs = configs[:1]
    return configs
...
def _mhc_sinkhorn_knopp_fwd_fused(
    x_ptr, # (M, n*n)
    output_ptr,
    hist_f_ptr, hist_g_ptr,# Assume this is contiguous and laid out as (iters+1, M, n)
    stride_xm, stride_xn,
    stride_out_m, stride_out_n,
    M, n: tl.constexpr,
    ...
    for iter_idx in range(iters):
        ...
        f_hist_ptrs = hist_f_ptr + (iter_idx + 1) * BTn + offs_batch[:, None] * n + offs_n_hist[None, :]
        tl.store(f_hist_ptrs, f, mask=mask_batch[:, None])
        ...
        g_hist_ptrs = hist_g_ptr + (iter_idx + 1) * BTn + offs_batch[:, None] * n + offs_n_hist[None, :]
        tl.store(g_hist_ptrs, g, mask=mask_batch[:, None])

    log_P = f[:, :, None] + x + g[:, None, :]
    log_P = tl.reshape(log_P, (BATCH_SIZE, n * n,))
    P = tl.exp(log_P)
...
def _mhc_sinkhorn_knopp_bwd_fused(
    grad_out_ptr, output_ptr,
    grad_x_ptr, x_ptr,
    hist_f_ptr, hist_g_ptr, # Assume this is contiguous and laid out as (iters+1, M, n)
    stride_grad_out_m, stride_grad_out_n,
    stride_out_m, stride_out_n,
    stride_grad_xm, stride_grad_xn,
    stride_xm, stride_xn,
    M, n: tl.constexpr,
    BLOCK_SIZE: tl.constexpr,
    iters
    ...
    grad_out_ptrs = grad_out_ptr + offs_batch[:, None] * stride_grad_out_m + offs_nn[None, :] * stride_grad_out_n
    ...
    grad_x_ptrs = grad_x_ptr + offs_batch[:, None] * stride_grad_xm + offs_nn[None, :] * stride_grad_xn
    tl.store(grad_x_ptrs, tl.reshape(grad_x, (BATCH_SIZE, n * n,)), mask=mask_batch[:, None])
...
def _mhc_pre_fwd(
    ...
    stride_H_pre_m, stride_H_pre_n,
    stride_output_m, stride_output_c,
    # Meta-parameters
    BLOCK_SIZE_M: tl.constexpr,
    ...
    x = tl.load(x_ptrs, mask=mask_m[:, None, None] & mask_c[None, None, :], other=0.0) # (BLOCK_SIZE_M, n, BLOCK_SIZE_C)
    ...
    H_pre = tl.reshape(H_pre, (BLOCK_SIZE_M, n, 1)) # (BLOCK_SIZE_M, n, 1), which is the same as (BLOCK_SIZE_M, 1, n) after transpose
...
def _mhc_pre_bwd(
    grad_output_ptr, # (M, C)
    H_pre_ptr, grad_H_pre_ptr, # H_pre_ptr points to a slice of the parent tensor H, and grad_H_pre_ptr points to a contiguous tensor (M, n)
    x_ptr, grad_x_ptr, # # (M, n, C)
    M, C,
    n: tl.constexpr,
    stride_grad_output_m, stride_grad_output_c,
    stride_H_pre_m, stride_H_pre_n,
    stride_grad_H_pre_m, stride_grad_H_pre_n,
    stride_xm, stride_xn, stride_xc,
    stride_grad_xm, stride_grad_xn, stride_grad_xc,
    # Meta-parameters
    BLOCK_SIZE_M: tl.constexpr,
    BLOCK_SIZE_C: tl.constexpr,
):
    ...
    grad_output_ptrs = grad_output_ptr + offs_m[:, None] * stride_grad_output_m + offs_c[None, :] * stride_grad_output_c
    grad_output = tl.load(grad_output_ptrs, mask=mask_m[:, None] & mask_c[None, :], other=0.0) # (BLOCK_SIZE_M, BLOCK_SIZE_C)

    H_pre_offs = pid_m * BLOCK_SIZE_M * n + tl.arange(0, BLOCK_SIZE_M * n)
    H_pre = tl.load(H_pre_ptr + H_pre_offs, mask=H_pre_offs < M * n, other=0.0) # (BLOCK_SIZE_M * n)
    ...
    grad_x_ptrs = grad_x_ptr + offs_m[:, None, None] * stride_grad_xm + offs_n[None, :, None] * stride_grad_xn + offs_c[None, None, :] * stride_grad_xc
    ...
    grad_H_pre = tl.sum((tl.reshape(grad_output, (BLOCK_SIZE_M, 1, BLOCK_SIZE_C)) * x).to(tl.float32), axis=2) # (BLOCK_SIZE_M, n)
...
def _mhc_post_res_fwd(
    ...
    n: tl.constexpr,
    stride_fm, stride_fc,
    stride_H_post_m, stride_H_post_n,
    stride_xm, stride_xn, stride_xc,
    stride_H_res_m, stride_H_res_n1, stride_H_res_n2,
    stride_output_m, stride_output_n, stride_output_c,
    # Meta-parameters
    BLOCK_SIZE_M: tl.constexpr,
    BLOCK_SIZE_C: tl.constexpr,
):
    ...
    output_ptrs = output_ptr + offs_m[:, None, None] * stride_output_m + offs_n[None, :, None] * stride_output_n + offs_c[None, None, :] * stride_output_c
...
def _mhc_post_res_bwd(
    ...
    M, C,
    n: tl.constexpr,
    stride_grad_output_m, stride_grad_output_n, stride_grad_output_c,
    stride_fm, stride_fc,
    stride_H_post_m, stride_H_post_n,
    stride_xm, stride_xn, stride_xc,
    stride_H_res_m, stride_H_res_n1, stride_H_res_n2,
    stride_grad_H_post_m, stride_grad_H_post_n,
    stride_grad_fm, stride_grad_fc,
    stride_grad_H_res_m, stride_grad_H_res_n1, stride_grad_H_res_n2,
    stride_grad_xm, stride_grad_xn, stride_grad_xc,
    # Meta-parameters
    BLOCK_SIZE_M: tl.constexpr, 
    BLOCK_SIZE_C: tl.constexpr,
    ...
    tl.assume(stride_grad_xm > 0 and stride_grad_xn > 0 and stride_grad_xc == 1)
    tl.assume(stride_grad_H_res_m == n*n and stride_grad_H_res_n1 == n and stride_grad_H_res_n2 == 1)
    ...
    H_res_offs = pid_m * BLOCK_SIZE_M * n * n + tl.arange(0, BLOCK_SIZE_M * n * n)
    H_res = tl.load(H_res_ptr + H_res_offs, mask=H_res_offs < M * n * n, other=0.0) # (BLOCK_SIZE_M, n, n)
    H_res = tl.reshape(H_res, (BLOCK_SIZE_M, n, n)) # (BLOCK_SIZE_M, n, n)

    grad_out_ptrs = grad_output_ptr + offs_m[:, None, None] * stride_grad_output_m + offs_n[None, :, None] * stride_grad_output_n + offs_c[None, None, :] * stride_grad_output_c
    grad_out = tl.load(grad_out_ptrs, mask=mask_m[:, None, None] & mask_c[None, None, :], other=0.0) # (BLOCK_SIZE_M, n, BLOCK_SIZE_C)
    grad_out_T = tl.trans(grad_out, (0, 2, 1)) # (BLOCK_SIZE_M, n, BLOCK_SIZE_C) -> (BLOCK_SIZE_M, BLOCK_SIZE_C, n)
    grad_out_T = tl.reshape(grad_out_T, (BLOCK_SIZE_M, BLOCK_SIZE_C, 2, 2)) # (BLOCK_SIZE_M, BLOCK_SIZE_C, 2, 2)
    grad_out_T01, grad_out_T23 = tl.split(grad_out_T) # (BLOCK_SIZE_M, BLOCK_SIZE_C, 2), (BLOCK_SIZE_M, BLOCK_SIZE_C, 2)
    grad_out_T0, grad_out_T1 = tl.split(grad_out_T01) # (BLOCK_SIZE_M, BLOCK_SIZE_C), (BLOCK_SIZE_M, BLOCK_SIZE_C)
    grad_out_T2, grad_out_T3 = tl.split(grad_out_T23) # (BLOCK_SIZE_M, BLOCK_SIZE_C), (BLOCK_SIZE_M, BLOCK_SIZE_C)
    ...
    grad_H_post = tl.reshape(tl.join(grad_H_post01, grad_H_post23), (BLOCK_SIZE_M * n)) # (BLOCK_SIZE_M, n)
    ...
    grad_H_res_T0 = tl.sum((x * grad_out_T0[:, None, :]).to(tl.float32), axis=2) # (BLOCK_SIZE_M, n)
    grad_H_res_T1 = tl.sum((x * grad_out_T1[:, None, :]).to(tl.float32), axis=2) # (BLOCK_SIZE_M, n)
    grad_H_res_T2 = tl.sum((x * grad_out_T2[:, None, :]).to(tl.float32), axis=2) # (BLOCK_SIZE_M, n)
    grad_H_res_T3 = tl.sum((x * grad_out_T3[:, None, :]).to(tl.float32), axis=2) # (BLOCK_SIZE_M, n)
    ...
    grad_H_res_T = tl.reshape(tl.join(grad_H_res_T01, grad_H_res_T23), (BLOCK_SIZE_M, n, n)) # (BLOCK_SIZE_M, n, 4)
