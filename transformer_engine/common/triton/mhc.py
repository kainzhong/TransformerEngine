...
def _mhc_projection_bwd_fused(
    ...
    grad_h_ptrs = grad_h_ptr + offs_m[:, None] * stride_grad_hm + offs_n_full[None, :] * stride_grad_hn
    grad_h = tl.load(
    ...
    grad_ms_ptrs = grad_ms_ptr + offs_r * stride_grad_ms
...
@triton.autotune(
    configs=expand_combine_config(), key=["M", "C"], reset_to_zero=["grad_H_post_ptr", "grad_H_res_ptr"]
)
@triton.jit
