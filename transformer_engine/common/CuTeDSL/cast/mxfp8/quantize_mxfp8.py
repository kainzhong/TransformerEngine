...
class MXFP8QuantizeSpecializedRowwiseKernel(MXFP8QuantizeKernelBase):
    ...
    def __call__(
        ...
        # If WITH_GEMM_SWIZZLED_SCALES is enabled, express the output scale tensor with
        # cublas's swizzled layout so it's transparent to the rest of the kernel. See
        # https://docs.nvidia.com/cuda/cublas/#d-block-scaling-factors-layout for details.
    ...
    def _kernel_main(
        ...
        if cutlass.const_expr(self._STASH_SCALE_TO_SMEM):
            ...
            if cutlass.const_expr(self.cfg.WITH_GEMM_SWIZZLED_SCALES):
                # The GEMM-swizzled scale layout is only contiguous (stride 1) within a
                # 4-column swizzle tile (GEMM_SWIZZLED_SCALE_TILE_DIM_X); a wider vectorized
                # store would straddle non-contiguous swizzle-tile boundaries.
