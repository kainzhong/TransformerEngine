...
class DLTensorWrapper : public DLTensor {
 public:
  ...
  explicit DLTensorWrapper(const NVTEBasicTensor &tensor, bool flatten_2D = true,
    ...
    // If the dtype has lanes > 1, the innermost extent must be packed as the shape and strides
    // are measured in the units of the packed dtype. The packed dtype will be unpacked by CuTe DSL's
    // autogenerated TVM-FFI entrypoint, and the logical dtype, shape, and stride will be restored.
