...
"""Mixin class holding data specific for HybridTensor.

A hybrid tensor quantizes its two directions in (potentially) different
formats -- e.g. row-wise in NVFP4 and column-wise in MXFP8. Each direction
on its own is a bona fide single-format tensor, so format-specific operations
(dequantize, GEMM) are expected to be served by emitting an ephemeral
single-format view over one direction's buffers and delegating to the existing
NVFP4 / MXFP8 machinery. The data members and structural plumbing live here.
"""
...
class _FromHybridFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        raise ValueError("Cannot dequantize Hybrid tensor with no data")
...
class HybridTensorStorage(QuantizedTensorStorage):
    """Mixin class that holds data attributes of HybridTensor.

    HybridTensor inherits from the PyTorch tensor class and this mixin
    ...
    # Builder class for casting to the hybrid format
    ...
    def copy_from_storage(self, src: QuantizedTensorStorage) -> None:
        """Copy data buffers from another HybridTensorStorage."""
        if not isinstance(src, HybridTensorStorage):
            raise TypeError("copy_from_storage expects HybridTensorStorage")
        if self._dtype_row != src._dtype_row or self._dtype_column != src._dtype_column:
            raise RuntimeError("Hybrid dtype mismatch in copy_from_storage")
    ...
    def prepare_for_saving(self) -> Tuple[list[Optional[torch.Tensor]], HybridTensorStorage]:
    ...
    def size(self, dim: Optional[int] = None) -> Union[torch.Size, int]:
        ...
        if shape is None:
            raise RuntimeError("Attempted to get shape of Hybrid tensor with no data")
        if dim is None:
    ...
    def device(self):
        ...
        raise RuntimeError("HybridTensorStorage has no data!")
    ...
    def update_usage(
        ...
        if rowwise_usage:
            if self._rowwise_data is None:
                raise RuntimeError(
                    "Requested row-wise usage, but HybridTensor is missing row-scaled data"
                )
            if self._rowwise_scale_inv is None:
                raise RuntimeError(
                    "Requested row-wise usage, but HybridTensor is missing row-scaled scale-inverses"
                )
            if self._amax_rowwise is None and self.is_nvfp4_dtype(self._dtype_row):
                raise RuntimeError(
                    "Requested row-wise NVFP4 usage, but HybridTensor is missing per tensor"
        ...
        if columnwise_usage:
            if self._columnwise_data is None:
                raise RuntimeError(
                    "Requested column-wise usage, but HybridTensor is missing column-scaled data"
                )
            if self._columnwise_scale_inv is None:
                raise RuntimeError(
                    "Requested column-wise usage, "
                    "but HybridTensor is missing column-scaled scale-inverses"
                )
            if self._amax_columnwise is None and self.is_nvfp4_dtype(self._dtype_column):
                raise RuntimeError(
                    "Requested column-wise NVFP4 usage, "
                    "but HybridTensor is missing per tensor column-scaled scale-inverse"
                )
        else:
            self._columnwise_data = None
            self._columnwise_scale_inv = None
