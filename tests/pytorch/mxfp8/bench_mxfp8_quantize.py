...
# Gated ops take a 2N-wide [gate, up] input and route through the separate
# quantize_gated_mxfp8_kernel in cast/mxfp8/gated_mxfp8.cuh.
_GATED_FNS = {g: getattr(tex, g) for g in ("swiglu", "geglu")}
_DGATED_FNS = {"d" + g: getattr(tex, "d" + g) for g in _GATED_FNS}

# combo -> kind. `kind` picks the tex entry point and how many inputs are read.
COMBO_KIND = {"plain": "plain", "dbias": "dbias"}
for _g in _GATED_FNS:
    COMBO_KIND[_g] = "gated"
    COMBO_KIND["d" + _g] = "dgated"
for _a in _ACT_FNS:
    COMBO_KIND[_a] = "act"
...
def make_fn(combo, x, act_input, quantizer):
    ...
    if kind == "gated":
        op = _GATED_FNS[combo]
        return lambda: op(x, quantizer)  # x is the 2N-wide gated input
    if kind == "dgated":
        op = _DGATED_FNS[combo]
        return lambda: op(act_input, x, quantizer)  # act_input is grad, x is 2N-wide
    raise ValueError(f"unknown combo {combo!r}")


def _bytes(M, in_bytes_per_row, out_N, rowwise, colwise):
    total = M * in_bytes_per_row
    if rowwise:
        total += M * out_N + M * (out_N // 32)
    if colwise:
        total += M * out_N + (M // 32) * out_N
    return total


def workload_bytes(M, N, in_bytes_per_elt, need_act_input, rowwise, colwise, kind="plain"):
    """Lower bound on HBM traffic: inputs read + FP8 out + e8m0 scales written."""
    if kind == "gated":
        return _bytes(M, 2 * N * in_bytes_per_elt, N, rowwise, colwise)
    if kind == "dgated":
        return _bytes(M, (N + 2 * N) * in_bytes_per_elt, 2 * N, rowwise, colwise)
...
def run_workload(M, N, dir_name, combo, args, manifest):
    ...
    kind = COMBO_KIND[combo]
    need_act_input = kind in ("dact", "dbias_dact")

    torch.manual_seed(0)
    if kind in ("gated", "dgated"):
        x = torch.randn(M, 2 * N, dtype=in_dt, device="cuda")
        act_input = torch.randn(M, N, dtype=in_dt, device="cuda") if kind == "dgated" else None
    else:
    ...
    nbytes = workload_bytes(M, N, x.element_size(), need_act_input, rowwise, colwise, kind)
    manifest.append(
        {
            "range": rng,
            "combo": combo,
