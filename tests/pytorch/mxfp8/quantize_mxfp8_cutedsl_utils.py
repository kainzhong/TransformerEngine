...
def quantize_rowwise_mxfp8(
    ...
    if cutlass.const_expr(_row_fast):
        ...
        if cutlass.const_expr(ACTIVATION is not None):
            op = _ACTIVATIONS[ACTIVATION]
...
def quantize_colwise_mxfp8(
    ...
    HALF_PRECISION_PATH = _is_packed16(DTYPE) and ACTIVATION is None
    ...
    if cutlass.const_expr(HALF_PRECISION_PATH):
        ...
        # Apply activation in f32 (no truncation yet)
        if cutlass.const_expr(ACTIVATION is not None):
            op = _ACTIVATIONS[ACTIVATION]
            for i in cutlass.range_constexpr(SCALE_DIM):
...
# ---------------------------------------------------------------------------
# NVFP4 quantization (pure cast, no activations / no stochastic rounding /
# no 2D-block scaling). Layout choices below are deliberately the simplest
# thing that lets these reuse the MXFP8 input SRAM tile so a hybrid kernel
# can call MXFP8 rowwise/colwise *and* NVFP4 rowwise/colwise on the same
# tile in shared memory.
#
# Two-level scaling (matches TE C++):
#   S_enc  = 448 * 6 / global_amax     (precomputed by the caller, uniform)
#   S_dec  = 1 / S_enc
#   per scale-block of SCALE_DIM_NVFP4 elements:
#     S_dec_b_fp8     = fp8e4m3(block_amax * (S_enc / 6))         → gmem
#     block_scale_inv = min(1 / (f32(S_dec_b_fp8) * S_dec), FLT_MAX)
#     out[i]          = fp4(in[i] * block_scale_inv)              → smem
#
# Output tile shapes (fp4 packs 2 per byte):
#   rowwise  : (TILE_Y,         TILE_X // 2)  uint8  — pairs along X
#   colwise  : (TILE_Y // 2,    TILE_X    )  uint8   — pairs along Y
# These shapes pair the two fp4 nibbles in the same dimension the scale
# block lives in, which keeps each thread's writes simple. The caller can
# transpose at TMA-store time if a different on-disk layout is needed.
# ---------------------------------------------------------------------------
SCALE_DIM_NVFP4 = 16


@cute.jit
def quantize_rowwise_nvfp4(
    sX_tile,        # (TILE_Y, TILE_X) bf16/fp16 smem view, post-TMA
    sO_row_tile,    # (TILE_Y, TILE_X // 2) uint8 smem view (rowwise FP4 output)
    mS_row_stage,   # (TILE_Y, TILE_X // SCALE_DIM) uint8 — one E4M3 byte per (row, scale-block)
    S_enc,          # Float32 — precomputed global encode scale (uniform across threads)
    tile_row_start, # Int32 — global row index of this stage's row 0
    tile_col_start, # Int32 — global col index of this CTA's col 0
    M, N,           # Int32 — full tensor extents; OOB threads skip scale store
    DTYPE,
    TILE_Y,
    TILE_X,
    SCALE_DIM,      # = SCALE_DIM_NVFP4 (16); explicit for symmetry with MXFP8 fn
):
    """Rowwise NVFP4 pass — reuses the MXFP8 rowwise 64-thread layout.

    Thread `(row, seg) = (tidx // 2, tidx % 2)` owns one `SEG = TILE_X // 2`
    (=32) element segment of row `row`, columns `[seg*SEG : seg*SEG+SEG]`.
    With MXFP8 `SCALE_DIM=32` that segment is exactly one block; with NVFP4
    `SCALE_DIM=16` it is `BLOCKS_PER_SEG = SEG // SCALE_DIM` (=2) blocks, so
    we loop over them — symmetric with `quantize_colwise_nvfp4`'s per-column
    `BLOCKS_PER_COL` loop. This keeps rowwise NVFP4 on the same 64-thread
    CTA as every other pass instead of needing 128 threads.
    """
    tidx, _, _ = cute.arch.thread_idx()

    SEG = TILE_X // 2                 # 32 — elements per thread (half a row)
    BLOCKS_PER_SEG = SEG // SCALE_DIM  # 2 — NVFP4 scale-blocks in that segment

    # Same TV layout as MXFP8 rowwise (thr (TILE_Y, 2), val (1, SEG)); the
    # output view packs 2 fp4 per byte so its val extent is SEG // 2.
    _, tv_layout_in = cute.make_layout_tv(
        thr_layout=cute.make_layout((TILE_Y, 2), stride=(2, 1)),
        val_layout=cute.make_layout((1, SEG), stride=(0, 1)),
    )
    _, tv_layout_out = cute.make_layout_tv(
        thr_layout=cute.make_layout((TILE_Y, 2), stride=(2, 1)),
        val_layout=cute.make_layout((1, SEG // 2), stride=(0, 1)),
    )
    sX_tv = cute.composition(sX_tile, tv_layout_in)
    sO_tv = cute.composition(sO_row_tile, tv_layout_out)

    sX_thread = sX_tv[tidx, None]   # (SEG,) bf16/fp16
    sO_thread = sO_tv[tidx, None]   # (SEG // 2,) uint8

    row = tidx // 2
    seg = tidx % 2

    # Packed views: read 2 elts/Int32, write 2 fp4-bytes/Int16.
    kit = _packed16_kit(DTYPE)
    sX_thread_i32 = cute.make_tensor(
        cute.recast_ptr(sX_thread.iterator, dtype=Int32),
        cute.make_layout((SEG // 2,), stride=(1,)),
    )
    sO_thread_i16 = cute.make_tensor(
        cute.recast_ptr(sO_thread.iterator, dtype=Int16),
        cute.make_layout((SEG // 4,), stride=(1,)),
    )

    S_dec = Float32(1.0) / S_enc
    fp4_max_inv = Float32(FP4_E2M1_MAX_RCP)

    # Loop over the BLOCKS_PER_SEG scale-blocks within this thread's segment.
    # Each block: amax over SCALE_DIM elements → E4M3 scale byte →
    # block_scale_inverse → SCALE_DIM/4 fp4x4 stores. Identical math to the
    # colwise path, just along X with the packed-x2 amax fast read.
    for blk in cutlass.range_constexpr(BLOCKS_PER_SEG):
        i32_base = blk * (SCALE_DIM // 2)   # 8 Int32 per block
        in_r = [None] * (SCALE_DIM // 2)
        for w in cutlass.range_constexpr(SCALE_DIM // 2):
            in_r[w] = sX_thread_i32[i32_base + w]

        # 1. amax via packed-x2 max.xorsign.abs
        amax_2x = Int32(0)
        for w in cutlass.range_constexpr(SCALE_DIM // 2):
            amax_2x = kit.abs_max_x2(amax_2x, in_r[w])
        amax_r = cute.arch.fmax(
            fabs_f32(kit.x2_lo_to_f32(amax_2x)),
            fabs_f32(kit.x2_hi_to_f32(amax_2x)),
        )

        # 2. Per-block E4M3 scale (saturating cvt → block_amax=0 ⇒ fp8 zero).
        S_dec_b_f32_pre = amax_r * (S_enc * fp4_max_inv)
        S_dec_b_byte = cvt_f32_to_fp8e4m3(S_dec_b_f32_pre)

        # Scale-block index within the row: seg covers blocks
        # [seg*BLOCKS_PER_SEG : +BLOCKS_PER_SEG].
        sb = seg * BLOCKS_PER_SEG + blk
        scale_row = tile_row_start + row
        scale_col_first_elt = tile_col_start + sb * SCALE_DIM
        if scale_row < M and scale_col_first_elt < N:
            mS_row_stage[(row, sb)] = Uint8(S_dec_b_byte)

        # 3. block_scale_inverse = min(1 / (f32(S_dec_b_fp8) * S_dec), FLT_MAX).
        S_dec_b_f32 = cvt_fp8e4m3_to_f32(S_dec_b_byte)
        block_scale_inverse = cute.arch.fmin(
            Float32(1.0) / (S_dec_b_f32 * S_dec), Float32(FP32_MAX))
        scale_2x = pack_f32x2(block_scale_inverse, block_scale_inverse)

        # 4. Cast SCALE_DIM elements → SCALE_DIM/4 × fp4x4 (2 bytes each).
        # Block blk occupies output bytes [blk*(SCALE_DIM/2) : +SCALE_DIM/2],
        # i.e. Int16 slots [blk*(SCALE_DIM/4) : +SCALE_DIM/4].
        i16_base = blk * (SCALE_DIM // 4)
        for w in cutlass.range_constexpr(SCALE_DIM // 4):
            packed_4 = pack_i32x2(in_r[2 * w], in_r[2 * w + 1])
            quad = kit.mul_cvt_to_fp4x4(packed_4, scale_2x)
            sO_thread_i16[i16_base + w] = _trunc_i32_to_i16(quad)

    return


@cute.jit
def quantize_colwise_nvfp4(
    sX_tile,        # (TILE_Y, TILE_X) bf16/fp16 smem view, post-TMA
    sO_col_tile,    # (TILE_X, TILE_Y // 2) uint8 smem view — TRANSPOSED FP4 output
                    # (row `tidx` = column `tidx` of input; bytes pack vertically
                    # adjacent input rows). Matches TE's NVFP4 columnwise data
                    # storage shape `(N, M // 2)`, so the caller's TMA S2G goes
                    # straight to the right gmem layout with no extra transpose.
    mS_col_stage,   # (TILE_X, TILE_Y // SCALE_DIM) uint8 — one E4M3 byte per
                    # (col, scale-block-y). Also transposed to match TE's NVFP4
                    # columnwise scale shape `(N, M // 16)`.
    S_enc,          # Float32 — precomputed global encode scale
    tile_row_start, # Int32 — global row index of this stage's row 0
    tile_col_start, # Int32 — global col index of this CTA's col 0
    M, N,
    DTYPE,
    TILE_X,
    TILE_Y,
    SCALE_DIM,      # = SCALE_DIM_NVFP4 (16)
):
    tidx, _, _ = cute.arch.thread_idx()
    BLOCKS_PER_COL = TILE_Y // SCALE_DIM  # e.g. 2 for MXFP8's TILE_Y=32 reused

    # Input thread layout — same `(1, TILE_X)` as MXFP8 colwise so the
    # launch reuses TILE_X threads. Each thread owns one full input column
    # (TILE_Y elements).
    _, tv_layout_in = cute.make_layout_tv(
        thr_layout=cute.make_layout((1, TILE_X), stride=(TILE_X, 1)),
    ...
    sX_tv = cute.composition(sX_tile, tv_layout_in)
    sX_thread = sX_tv[tidx, None]    # (TILE_Y,) bf16/fp16, column tidx

    # The per-stage output tile arrives as a rank-1 nested mode (like the MXFP8
    # smem tiles); rebuild a flat rank-2 (TILE_X, TILE_Y // 2) view over the same
    # bytes so the (col, byte) stores below index cleanly.
    sO_col_2d = cute.make_tensor(
        sO_col_tile.iterator,
        cute.make_layout((TILE_X, TILE_Y // 2), stride=(TILE_Y // 2, 1)),
    )

    # Column elements are TILE_X apart in memory → widen to f32 once and
    # do everything (amax, mul, fp4 cast) from f32 registers.
    sX_thread_f32 = cute.make_rmem_tensor(
        layout_or_shape=cute.make_layout((TILE_Y,), stride=(1,)),
        dtype=Float32,
    )
    if cutlass.const_expr(_is_packed16(DTYPE)):
        # Widen via bits → f32 (avoids `Float32(bf16)` going through a
        # generic conversion path; matches the MXFP8 colwise fast read).
        kit = _packed16_kit(DTYPE)
        sX_thread_i16 = cute.make_tensor(
            cute.recast_ptr(sX_thread.iterator, dtype=Int16),
            cute.make_layout((TILE_Y,), stride=(TILE_X,)),
        )
        for i in cutlass.range_constexpr(TILE_Y):
            sX_thread_f32[i] = kit.bits_to_f32(sX_thread_i16[i])
    else:
        for i in cutlass.range_constexpr(TILE_Y):
            sX_thread_f32[i] = Float32(sX_thread[i])
    ...
    # Loop over the BLOCKS_PER_COL scale-blocks down the column. Each
    # iteration is a self-contained NVFP4 quantization of SCALE_DIM
    # elements: amax → scale byte → block_scale_inv → SCALE_DIM/4 fp4x4
    # stores. The byte CONTENTS pack vertical pairs of input elements
    # (2i, 2i+1) of column tidx; storage shape is transposed so writes are
    # contiguous along the row axis of `sO_col_tile`.
    for blk in cutlass.range_constexpr(BLOCKS_PER_COL):
        base = blk * SCALE_DIM
        # 1. amax over this scale block
        amax_c = Float32(0.0)
        for i in cutlass.range_constexpr(SCALE_DIM):
            amax_c = cute.arch.fmax(amax_c, fabs_f32(sX_thread_f32[base + i]))

        # 2. Per-block E4M3 scale
        S_dec_b_f32_pre = amax_c * (S_enc * fp4_max_inv)
        S_dec_b_byte = cvt_f32_to_fp8e4m3(S_dec_b_f32_pre)

        # OOB guard: this thread's column extent vs N, this block's row
        # extent vs M. Transposed scale tensor — col coord first.
        scale_row_first = tile_row_start + base
        scale_col = tile_col_start + tidx
        if scale_row_first < M and scale_col < N:
            mS_col_stage[(tidx, blk)] = Uint8(S_dec_b_byte)

        # 3. block_scale_inverse (same min/FLT_MAX clamp as rowwise)
        S_dec_b_f32 = cvt_fp8e4m3_to_f32(S_dec_b_byte)
        block_scale_inverse = cute.arch.fmin(
            Float32(1.0) / (S_dec_b_f32 * S_dec), Float32(FP32_MAX))
        scale_2x = pack_f32x2(block_scale_inverse, block_scale_inverse)

        # 4. Cast SCALE_DIM elements → SCALE_DIM/4 × fp4x4. Output is
        # transposed `(TILE_X, TILE_Y // 2)` uint8, so the two bytes of
        # each fp4x4 land at consecutive byte offsets along row `tidx` —
        # we can write Int16 directly instead of two scattered byte stores.
        # Recast row `tidx` to an Int16 view of length TILE_Y // 4 per
        # full thread (or SCALE_DIM // 4 per scale-block iter).
        for w in cutlass.range_constexpr(SCALE_DIM // 4):
            in01 = pack_f32x2(sX_thread_f32[base + 4 * w],
                              sX_thread_f32[base + 4 * w + 1])
            in23 = pack_f32x2(sX_thread_f32[base + 4 * w + 2],
                              sX_thread_f32[base + 4 * w + 3])
            quad = mul_cvt_f32x4_to_fp4x4(in01, in23, scale_2x)
            # quad low 16 bits: byte0 = (fp4(elt 4w+1) << 4) | fp4(elt 4w+0),
            # byte1 = (fp4(elt 4w+3) << 4) | fp4(elt 4w+2). Pair (4w, 4w+1) →
            # transposed-output col `blk*(SCALE_DIM/2) + 2w`; pair (4w+2, 4w+3) →
            # col `blk*(SCALE_DIM/2) + 2w + 1`.
            byte_lo = quad & Int32(0xFF)
            byte_hi = (quad >> Int32(8)) & Int32(0xFF)
            out_col = blk * (SCALE_DIM // 2) + 2 * w
            sO_col_2d[(tidx, out_col)] = Uint8(byte_lo)
            sO_col_2d[(tidx, out_col + 1)] = Uint8(byte_hi)

    # No single-scalar amax return for colwise — multiple scale blocks
    # per thread per call leaves no canonical value (and the PoC caller
    # doesn't need it). Matches the C++ NVFP4 colwise path which does
    # the cross-block aggregation outside.
    return
