...
BENCH_OVERRIDE = None  # set by --bench
...
def run_nsys(label, outdir, bench_args, env_extra=None):
    ...
    cmd = [
        ...
        sys.executable, BENCH_OVERRIDE or BENCH,
        "--manifest", manifest,
    ] + bench_args
    r = sh(cmd, env=env)
...
def main():
    ...
    p.add_argument("--bench", default=None,
                   help="workload script to profile (default: bench_mxfp8_quantize.py)")
    ...
    if args.bench:
        globals()["BENCH_OVERRIDE"] = os.path.join(HERE, args.bench)
